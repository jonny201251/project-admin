...
public class InContractServiceImpl extends ServiceImpl<InContractMapper, InContract> implements InContractService {
    ...
    private void change(InContract formValue) {
        InContract inContract = this.getById(formValue.getId());
        inContract.setHaveDisplay("否");
        this.updateById(inContract);
        //
        formValue.setId(null);
        formValue.setProcessInstId(null);
        formValue.setBeforeId(inContract.getId());
        formValue.setHaveDisplay("是");
        formValue.setVersion(formValue.getVersion() + 1);
        if (formValue.getBaseId() == null) {
            //第一次修改
            formValue.setBaseId(inContract.getId());
        } else {
            //第二、三、N次修改
    ...
    @Override
    public synchronized boolean btnHandle(InContractVO inContractVO) {
        SysUser user = (SysUser) httpSession.getAttribute("user");
        InContract formValue = inContractVO.getFormValue();
        String type = inContractVO.getType();
        String buttonName = inContractVO.getButtonName();
        String path = inContractVO.getPath();
        if (type.equals("add")) {
            ...
            String haveEditForm = inContractVO.getHaveEditForm();
            ...
            processInstNode.setDeptName(user.getDeptName());
            processInstNode.setButtonName(buttonName.substring(buttonName.lastIndexOf("_") + 1));
            if (ObjectUtil.isNotEmpty(inContractVO.getComment())) {
                processInstNode.setComment(inContractVO.getComment());
            }
            //
            HistoricTaskInstance historicTaskInstance = workFlowBean.getHistoricTaskInstance(actProcessInstanceId, task.getTaskDefinitionKey());
            Date startDateTime = historicTaskInstance.getStartTime();
            ...
            if (processInst == null) {
                ...
                this.remove(new LambdaQueryWrapper<InContract>().eq(InContract::getId, formValue.getId()));
            } else {
                //退回、退回申请人、申请人撤回
